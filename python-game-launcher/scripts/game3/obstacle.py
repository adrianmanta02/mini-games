...
class FormBarrier:
    """Bariera care permite trecerea doar intr-o anumita forma"""
    ...
    def __init__(self, x, y, width, height, allowed_form):
        ...
        self.width = width
        self.height = height
        self.allowed_form = allowed_form  # "light" sau "shadow"

        if allowed_form == "light":
            self.color = (255, 255, 100, 120)  # galben transparent
            self.border_color = YELLOW
        else:
            self.color = (100, 80, 150, 120)  # violet transparent
            self.border_color = SHADOW_PURPLE

    def can_pass(self, player_form):
        """returneaza True daca playerul poate trece"""
        return player_form == self.allowed_form

    def draw(self, screen, screen_movement):
        draw_x = self.x - screen_movement

        # bariera transparenta
...
class ShadowZone:
    """Zona de umbra - forteaza Shadow Form"""
...
class MovingPlatform(Platform):
    """Platforma care se misca sus-jos sau stanga-dreapta"""
