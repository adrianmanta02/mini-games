...
def draw_background_with_decorations(screen):
    # background with gradient + animated clouds + sun
...
def draw_spotlight_darkness(screen, firefly, screen_movement):
    # darkness with a spotlight only around the firefly

    # darkness starts only when light drops below 80%
    if firefly.light_level >= 80:
        return

    # calculate how dark it should be
    # at 80% light = 0% darkness
    # at 0% light = 100% darkness
    darkness_intensity = (80 - firefly.light_level) / 80
    ...
    # create circular gradient for the spotlight
    ...
    # apply darkness with spotlight "cut out"
...
def draw_ui(screen, firefly, level_id):
    ...
    # main text - black when the bar is full, white when empty
...
def draw_intro_screen(screen):
    ...
    # animated firefly in the center
    firefly_y = 370
...
while running:
    ...
    if not game_over and not win and not level_complete:
        ...
        # check collision with form barriers
